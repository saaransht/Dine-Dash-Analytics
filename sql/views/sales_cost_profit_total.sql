...
with ProductsSold as (
	select dr.restaurant_name, dp.product_id, dp.product_name, sum(dod.amount) as num_sold, round(sum(dod.amount) * dpp.price, 2) as sales_euro
	...
	group by dr.restaurant_name, dp.product_id, dp.product_name, dpp.price
),

RestaurantSales as (
	select restaurant_name, sum(sales_euro) as sales from ProductsSold
	group by restaurant_name
),

-- CALCULATING SALARIES BY RESTAURANT
WorkDay as (
	select es.employee_id, es.restaurant_id, date(es.timestamp), 
...
WorkDuration as (
	select employee_id, restaurant_id, start_time, end_time,
...
Salaries as (
	...
	group by wd.employee_id, dr.restaurant_name, ds.salary_h
),

RestaurantSalaries as (
	select restaurant_name, sum(salaries) as salaries
	from Salaries
	group by restaurant_name
),
...
IngredientAmounts as (
	select ps.restaurant_name, ps.product_id, dpi.ingredient_id, dpi.amount_kg * ps.num_sold as total_kg
	from ProductsSold ps
	join reporting.dim_product_ingredients as dpi on dpi.product_id = ps.product_id
),

IngredientCost as (
	select ia.restaurant_name, ia.ingredient_id, sum(ia.total_kg) as total_kg, sum(ia.total_kg) * dic.cost_kg as ingredient_cost
	from IngredientAmounts ia
	join reporting.dim_ingredient_cost dic on dic.ingredient_id = ia.ingredient_id
	group by ia.restaurant_name, ia.ingredient_id, dic.cost_kg
),

IngredientCostRestaurant as (
	select restaurant_name, sum(ingredient_cost) as ingredient_cost
	from IngredientCost
	group by restaurant_name
)
...
select icr.restaurant_name, 
	   ...
	   round(s.salaries, 2) as salaries_paid,
